"""
Simple SIMBAD Data Enhancement Module
...
class SIMBADEnhancer:
    ...
    def find_missing_simbad_rows(self, csv_path: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def query_simbad_cone(self, ra: float, dec: float, radius: float) -> Optional[Dict]:
        ...
        Uses direct HTTP requests to SIMBAD script interface.
        """
        # Check cache first
        ...
        if not REQUESTS_AVAILABLE:
            logger.warning("requests not available, cannot query SIMBAD")
            return None
        
        try:
            # Use SIMBAD script interface
...
"""
                ...
                if len(parts) >= 7:
                    ...
                    try:
                        ...
                        if ASTROPY_AVAILABLE and not (np.isnan(simbad_ra) or np.isnan(simbad_dec)):
                            ...
                            ra_diff = abs(ra - simbad_ra) * np.cos(np.radians((dec + simbad_dec) / 2))
                            dec_diff = abs(dec - simbad_dec)
                            match_distance = np.sqrt(ra_diff**2 + dec_diff**2) * 3600  # arcsec
    ...
    def get_source_name(self, simbad_data: Dict) -> str:
        ...
        if main_id:
            ...
            # Remove common prefixes for friendlier names, but keep scientific ones
    ...
    def enhance_single_row(self, row: pd.Series, row_index: int) -> Dict:
        ...
        if simbad_data:
            ...
            self.misses.append({
                ...
                'ra': ra,
                'dec': dec,
                'radius_tried': search_radius if 'search_radius' in locals() else 5.0
            })
            ...
            return {
                ...
                'source_name': '',
                'search_radius': search_radius if 'search_radius' in locals() else 5.0,
