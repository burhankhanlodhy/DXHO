...
class MainGUI:
    ...
    def _create_menu_bar(self):
        ...
        process_menu.add_command(label="Convert Image to FITS", command=self.convert_image_to_fits)
        process_menu.add_command(label="Batch Convert to FITS", command=self.batch_convert_to_fits)
    ...
    def _create_main_content(self):
        ...
        button_data = [
            ...
            ("Convert to FITS", self.convert_image_to_fits, "Convert any supported image to FITS format"),
    ...
    def convert_image_to_fits(self):
        """Convert any supported image format to FITS."""
        # Ask user to select image file
        supported_formats = [
            ("All supported images", "*.cr2;*.cr3;*.nef;*.arw;*.dng;*.raf;*.orf;*.rw2;*.pef;*.srw;*.jpg;*.jpeg;*.png;*.tiff;*.tif;*.bmp"),
            ("RAW formats", "*.cr2;*.cr3;*.nef;*.arw;*.dng;*.raf;*.orf;*.rw2;*.pef;*.srw"),
            ("Standard formats", "*.jpg;*.jpeg;*.png;*.tiff;*.tif;*.bmp"),
            ("All files", "*.*")
        ]
        
        filepath = filedialog.askopenfilename(
            title="Select Image File to Convert",
            filetypes=supported_formats
        )

        if filepath:
            # Check if format is supported
            if not self.conversion.is_supported_format(filepath):
                messagebox.showerror('Unsupported Format', 
                                   f'File format not supported: {os.path.splitext(filepath)[1]}')
                return
            ...
            if save_path:
                self._show_progress_window("Image to FITS Conversion")

                # Run conversion in a separate thread to prevent GUI freezing
                def conversion_thread():
                    success = self.conversion.convert_to_fits(filepath, save_path)
                    if success:
                        self._add_info_message(f"Image converted to FITS: {os.path.basename(save_path)}")

                threading.Thread(target=conversion_thread, daemon=True).start()

    def batch_convert_to_fits(self):
        ...
        filepaths = filedialog.askopenfilenames(
            title="Select Image Files for Batch Conversion",
            filetypes=supported_formats
        )
        ...
        if filepaths:
            ...
            output_dir = filedialog.askdirectory(
                title="Select Output Directory for FITS Files"
            )

            if output_dir:
                self._show_progress_window(f"Batch Conversion ({len(filepaths)} files)")

                # Run batch conversion in a separate thread
                def batch_conversion_thread():
                    successful, failed, failed_files = self.conversion.batch_convert_to_fits(filepaths, output_dir)
                    self._add_info_message(f"Batch conversion complete: {successful} successful, {failed} failed")
...
1. OPENING FILES
   ...
   • Supported formats: 
     ...
     - Standard: JPG, PNG, TIFF, BMP, GIF
     - Astronomical: FITS

2. IMAGE TO FITS CONVERSION
   • Select Process > Convert Image to FITS for universal conversion
   • Or use Process > Convert RAW to FITS for RAW-specific workflow
   • Choose any supported image format as input
   • Metadata is preserved where available (EXIF data)
   • Output is optimized for astronomical analysis

3. IMAGE STACKING
   • Select Process > Stack FITS Images
   • Choose multiple FITS files to combine
   • The mean-combined result reduces noise and improves SNR

4. APERTURE PHOTOMETRY
   • Open a FITS file for analysis
   • Select Analysis > Aperture Photometry
   • Set FWHM (typical: 3-8 pixels for ground-based images)
   • Set threshold (typical: 2-5 sigma above background)
   • Results are saved as CSV files with source coordinates and magnitudes

5. TIPS
   • Always convert to FITS format for scientific analysis
   • Stack multiple images to improve signal-to-noise ratio
   • RAW files provide the best quality for astrophotography
   • Adjust photometry parameters based on your seeing conditions
   • Check the information panel for processing updates and logs
