"""
SIMBAD Data Enhancement Module using astroquery
...
try:
    ...
    logger.info("Using astroquery for SIMBAD queries")
except ImportError:
    ASTROQUERY_AVAILABLE = False
    logger.warning("astroquery not available, using direct HTTP approach")
    import requests
...
class SIMBADEnhancer:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        if ASTROQUERY_AVAILABLE:
            # Configure Simbad instance
            self.simbad = Simbad()
            self.simbad.add_votable_fields('otype', 'sp', 'rv_value', 'distance')
            self.simbad.add_votable_fields('ra(d)', 'dec(d)')
        else:
            self.simbad = None
    ...
    def query_simbad_cone_astroquery(self, ra: float, dec: float, radius: float) -> Optional[Dict]:
        """
        Query SIMBAD using astroquery.
        ...
        try:
            ...
            radius_u = radius * u.arcsec
            
            # Query SIMBAD
            result_table = self.simbad.query_region(coord, radius=radius_u)
            ...
            if result_table is not None and len(result_table) > 0:
                ...
                # Extract data safely
                main_id = str(row['MAIN_ID']).strip() if 'MAIN_ID' in row.colnames else ''
                otype = str(row['OTYPE']).strip() if 'OTYPE' in row.colnames else ''
                sp_type = str(row['SP_TYPE']).strip() if 'SP_TYPE' in row.colnames else ''
                
                # Handle numeric fields
                try:
                    rv_value = float(row['RV_VALUE']) if 'RV_VALUE' in row.colnames and row['RV_VALUE'] is not np.ma.masked else np.nan
                except (ValueError, TypeError):
                    rv_value = np.nan
                
                try:
                    distance = float(row['Distance_distance']) if 'Distance_distance' in row.colnames and row['Distance_distance'] is not np.ma.masked else np.nan
                except (ValueError, TypeError):
                    distance = np.nan
                
                try:
                    simbad_ra = float(row['RA_d']) if 'RA_d' in row.colnames else np.nan
                    simbad_dec = float(row['DEC_d']) if 'DEC_d' in row.colnames else np.nan
                except (ValueError, TypeError):
                    simbad_ra = np.nan
                    simbad_dec = np.nan
            ...
            logger.warning(f"astroquery SIMBAD query failed for RA={ra}, Dec={dec}, radius={radius}: {e}")
            return None
    
    def query_simbad_cone_fallback(self, ra: float, dec: float, radius: float) -> Optional[Dict]:
        """
        Fallback SIMBAD query using direct HTTP requests.
        ...
        """
        try:
            # Use SIMBAD script interface as fallback
...
"""
            ...
            if data_lines:
                ...
                if len(parts) >= 7:
                    try:
                        ...
                        # Calculate match distance
                        if not (np.isnan(simbad_ra) or np.isnan(simbad_dec)) and ASTROQUERY_AVAILABLE:
                            coord1 = SkyCoord(ra*u.deg, dec*u.deg)
                            coord2 = SkyCoord(simbad_ra*u.deg, simbad_dec*u.deg) 
                            match_distance = coord1.separation(coord2).arcsec
                        else:
            ...
            logger.warning(f"Fallback SIMBAD query failed for RA={ra}, Dec={dec}, radius={radius}: {e}")
            return None
    ...
    def query_simbad_cone(self, ra: float, dec: float, radius: float) -> Optional[Dict]:
        ...
        if ASTROQUERY_AVAILABLE and self.simbad is not None:
            result = self.query_simbad_cone_astroquery(ra, dec, radius)
        
        # Fall back to direct HTTP if astroquery fails
        if result is None:
            result = self.query_simbad_cone_fallback(ra, dec, radius)
